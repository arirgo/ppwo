...
<?php
	...
	$sqlm	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='finish' and section ='".$akses['section']."'");
	$data1	=	mysqli_fetch_array($sqlm);
	$sqln	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='on process' and section ='".$akses['section']."'");
	$data2	=	mysqli_fetch_array($sqln);
	$sqlo	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='approved by IT' and section ='".$akses['section']."'");
	$data3	=	mysqli_fetch_array($sqlo);
	$sqlp	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='approved by SH' and section ='".$akses['section']."'");
	$data4	=	mysqli_fetch_array($sqlp);
	$sqlq	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='waiting' and section ='".$akses['section']."'");
	$data5	=	mysqli_fetch_array($sqlq);
	
	?>
		<tr>
				<td class="edit-record1">Finish</td>
				<td><?php echo $data1['jml'];?></td>
		</tr>
		<tr>
				<td class="edit-record2">On Process</td>
				<td><?php echo $data2['jml'];?></td>
		</tr>
		<tr>
				<td class="edit-record3">Approved By IT</td>
				<td><?php echo $data3['jml'];?></td>
		</tr>
		<tr>
				<td class="edit-record4">Approved By SH</td>
...
<?php include "menu-bawah.php"; ?>
	...
	<!--
				<td><a href = "javascript:  window.open('s_a_table.php?n=<?php //echo $dataq['status_wo']; ?>', 'winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=1200,height=500');" class = "btn btn-primary">Complete</a></td>
-->
        ...
        $(function(){
            ...
            $(document).on('click','.edit-record1',function(e){
             ...
             $(document).on('click','.edit-record3',function(e){
				window.open('s_a_table_sh_wo.php?n=approved by IT',
							'winpopup',
							'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=1200,height=500'
							);
            });
