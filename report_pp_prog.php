...
<div class="main-content">
	<div class="main-content-inner">
		...
		<?php if (isset($_POST['tombol'])){
				...
				?>
		<form action="report_pp_prog.php" method="post">
		...
		<table data-toggle="table"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="desc">
			<thead>
				...
				$atah = $_POST['tahun'];
				?>
				<a href="export_prog.php?tahun=<?php echo $atah; ?>&bulan=<?php echo $abul; ?>">Export ke Excell</a><br><br>							
				<th>No</th>								
				<th>Nomor PP</th>
				<th>Section</th>
				<th>Tgl Lapor</th>
		...
		<?php
			...
			$from	=	$_POST['from'];
				...
				$sql	=	mysqli_query($koneksi,"select * from view_prog where tgl_lapor like '".$thn."-".$bln."%' order by ".$urut." ".$from."");
				$no = 1;
		...
		<?php
			...
			$from	=	$_POST['from'];
				...
				$sql	=	mysqli_query($koneksi,"select * from view_prog where tgl_lapor like '".date('Y')."-".date('m')."%' order by no_pp ASC");
				$no = 1;
				while($data	=	mysqli_fetch_array($sql)){
					echo '<tr>';
					echo '<td>'.$no.'</td>';
