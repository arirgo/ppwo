...
<thead>
 ...
 <tr>
							...
							require_once("config.php");
								$result="SELECT * FROM tbl_wo where status_wo IN ('approved by SH','accepted by IT') and pls='$pls' order by tgl_permohonan desc";
								   ...
								   // mengulangi data agar tidak hanya 1 yang tampil
									...
									<?php
										...
										if($data['status_wo']=='waiting'){
												echo '<td><form name="f_take" action="take_wo.php" method="POST"><input type="hidden" name="txttake" value="'.$data['no_wo'].'"><input type="hidden" name="txtadmin" value="'.$nama.'"><button type="submit" class="btn btn-primary btn-xs" disabled>ACCEPT</button></form></td>';
											}else if($data['status_wo']=='approved by SH'){
												?>	
												<td><form name="f_take" action="take_wo.php" method="POST">
														...
														<input type="button" value="REJECT" class="btn btn-danger btn-xs" onclick="window.open('reject_window_it.php?kode=<?php echo $data['no_wo'];?>','winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=500,height=580');">
														</form>
