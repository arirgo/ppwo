...
<thead>
 ...
 <tr>
								<th data-sortable="true">No.</th>
									...
									<th  data-sortable="true">IT Area</th>
							...
							if($plsusr=="ho")
							{
								$result="SELECT * FROM tbl_pp where status_pp IN ('waiting') and pls='ho' ORDER BY status_pp DESC, tgl_lapor DESC";
							}else{
								$result="SELECT * FROM tbl_pp where status_pp IN ('waiting') and pls !='ho' ORDER BY status_pp DESC, tgl_lapor DESC";
							}
							//	$result="SELECT * FROM tbl_pp where status_pp IN ('waiting') ORDER BY status_pp DESC, tgl_lapor DESC";
								   ...
								   // mengulangi data agar tidak hanya 1 yang tampil
									...
									<?php
										...
										if($data['status_pp']=='waiting'){
												...
												<td>
												<input type="hidden" name="txttake" id="txttake" value="<?php echo $data['no_pp']; ?>">
														<input type="hidden" name="txtadmin" id="txtadmin" value="<?php echo $nama; ?>">
													<!-- <form name="f_take" action="take_pp.php" method="POST">
														<input type="hidden" name="txttake" value="<?php echo $data['no_pp']; ?>">
														<input type="hidden" name="txtadmin" value="<?php echo $nama; ?>">
														<?php
															$no_pp=$data['no_pp'];
															require_once("config.php");
															$sqldetail = mysqli_query($koneksi,"select * from tbl_pp where no_pp ='".$no_pp."'");
															$datadetail = mysqli_fetch_array($sqldetail);
														?>
														<input type="hidden" name="txtkerusahan" value="<?php echo $datadetail['kerusakan']; ?>">
														<input type="hidden" name="txtjenis" value="PP">
														<input type="hidden" name="group_project" value="PERBAIKAN"> -->
														<button type="button"  class="edit-record btn btn-primary btn-xs" data-id="<?php echo $data['no_pp']; ?>" >TAKE</button>
														<input type="button" class="btn btn-danger btn-xs" onclick="window.open('reject_pp.php?kode=<?php echo $data['no_pp'];?>','winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=500,height=580');" value="REJECT">
