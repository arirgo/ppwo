...
<thead>
 ...
 <tr>
							...
							require_once("config.php");
								$result="SELECT * FROM tbl_pp where status_pp IN ('on process','hold','finish','complete') ORDER BY status_pp DESC, tgl_lapor ASC";
								   ...
								   // mengulangi data agar tidak hanya 1 yang tampil
									...
									<?php
										...
										if($data['status_pp']=='waiting'){
											...
											}else if(($data['status_pp']=='finish')&&($data['pekerjaan']=='')){
												?><td><button type="submit" class="btn btn-primary btn-xs" onclick="window.open('finish_window.php?kode=<?php echo $data['no_pp'];?>','winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=600,height=580');">INPUT PEKERJAAN</button></td>
											<?php }else if(($data['status_pp']=='complete')&&($data['pekerjaan']=='')){
												?><td><button type="submit" class="btn btn-primary btn-xs" onclick="window.open('finish_window.php?kode=<?php echo $data['no_pp'];?>','winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=600,height=580');">INPUT PEKERJAAN</button></td>
											<?php
											}
										echo '</tr>';
