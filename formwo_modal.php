...
$cekan = mysqli_query($koneksi,"select * from user where nama ='".$atasnama."'");
$dtan = mysqli_fetch_array($cekan);
$an=$dtan['nama'];
...
<table width="80%" class="table table-striped table-bordered table-hover">
	...
	<tr>
		<td colspan="7" ><form action="pdfwo.php" method="post" >
		<input type="hidden" name="textwo" id="textwo" value="<?php echo $nowo; ?>">
		<button type="submit" class="btn btn-success btn-sm"><i class="ace-icon fa  fa-cloud-download align-top bigger-125"></i> Cetak</button>
		</form></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan="2"><?php echo $data['pemohon']." a/n : (".$data['an'].")";?></td>
		<td style="width: 1%;border:0;border-top:0;">&nbsp;</td>
	...
	<tr>
		<th style="width: 5%;">Jenis akses</th>
		<td colspan="2" style="width: 15%;"><?php 
		$jsakses = mysqli_query($koneksi,"select * from detail_akses  inner  join jenis_akses on detail_akses.akses =jenis_akses.idakses  where detail_akses.id_order='".$nowo."'");
		
		while($dtakses = mysqli_fetch_array($jsakses))
		{
		
			echo $dtakses['jenis_akses'].",";
		}
		
		?></td>
		<td style="width: 1%;border:0;border-top:0;">&nbsp;</td>
		<th style="width: 5%;">Sifat akses</th>
		<td style="width: 15%;" colspan="2"><?php echo $data['sifat']."( ".$data['exp']." )";?></td>
	</tr>
	<tr>
