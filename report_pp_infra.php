...
<div class="main-content">
	<div class="main-content-inner">
		...
		<?php if (isset($_POST['tombol'])){
				...
				?>
		<form action="report_pp_infra.php" method="post">
		<table width="80%" id="tblhis" border="0" class="table table-striped table-bordered table-hover">
			<tr>
				<td colspan="10" style=" background-color:#FF7C00;">Tabel Report</td>
			</tr>
			...
			<tr>
				...
				<td>
					...
					<option value="Mochamad Yusuf">Yusuf</option>
		...
		<table data-toggle="table"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="desc">
			<thead>
				...
				if($it=="infra")
				{
				echo	$infra="('Mochamad Yusuf','Nurul')";
				}else{
			echo		$infra="('".$it."')";
				}

				?>
				<a href="export_infra.php?tahun=<?php echo $atah; ?>&bulan=<?php echo $abul; ?>">Export ke Excell</a><br><br>							
				<th>No</th>								
				<th>Nomor PP</th>
				<th>Section</th>
				<th>Tgl Lapor</th>
		...
		<?php
			...
			$from	=	$_POST['from'];
				...
				$sql	=	mysqli_query($koneksi,"select * from tbl_pp where diterima in ('nurul','Mochamad Yusuf') and tgl_lapor like '".date('Y')."-".date('m')."%' order by tgl_lapor DESC");
				$no = 1;
				while($data	=	mysqli_fetch_array($sql)){
					$tgl1 = $data['tgl_lapor'].' '.$data['jam_lapor'];
					$tgl2 = $data['tgl_terima'].' '.$data['jam_terima'];
