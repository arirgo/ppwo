...
<div class="main-content">
	<div class="main-content-inner">
		...
		<?php if (isset($_POST['tombol'])){
				...
				?>
		<form action="report_pp.php" method="post">
		...
		<?php
			...
			$from	=	$_POST['from'];
				...
				$sql	=	mysqli_query($koneksi,"select * from tbl_pp where tgl_lapor like '".$thn."-".$bln."%' order by ".$urut." ".$from."");
				$no = 1;
		...
		<?php
			...
			$from	=	$_POST['from'];
				...
				$sql	=	mysqli_query($koneksi,"select * from tbl_pp where tgl_lapor like '".date('Y')."-".date('m')."%' order by no_pp ASC");
				$no = 1;
				while($data	=	mysqli_fetch_array($sql)){
					echo '<tr>';
					echo '<td>'.$no.'</td>';
