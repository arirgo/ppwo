		...
		<div class="page-content">
			...
			<div class="row">
				<div class="space-6"></div>
					<div class="col-sm-5">
						<div>
							<form action="main_teknik.php" method="post">
							<table width="60%" class="table table-striped table-bordered table-hover">
								...
								<?php
										...
										$que = mysqli_query($koneksi,"SELECT count(nama_komputer) as jmlpc from master_komputer and pls='$pls'");
...
<?php
	...
	$sqlt	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='hold' and pls='$pls' and tgl_permohonan like '".$thn."-".$bln."%'");
	...
	$sqlv	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='waiting' and pls='$pls' and tgl_permohonan like '".$thn."-".$bln."%'");
...
<div class="row">
	<div class="col-sm-6">
		<?php
			$syn = mysqli_query($koneksi,"select count(no_pp) as nopp from tbl_pp where status_pp = 'hold' and pls='$pls'");
			$datsyn = mysqli_fetch_array($syn);
			
		?>
		<table data-toggle="table"   data-sort-name="name" data-sort-order="desc">
			<thead>
			<tr>
			<th data-sortable="true">Outstanding Process PP</th>
			<th data-sortable="true">Jumlah</th>
			</tr>
			</thead>
			<tr>
				<td class="edit-record12">Hold</td>
				<td><?php echo $datsyn['nopp']; ?></td>
			</tr>
	...
	<div class="col-sm-6">
		<?php
			$syn = mysqli_query($koneksi,"select count(no_wo) as nowo from tbl_wo where status_wo = 'hold' and pls='$pls'");
			$datsyn = mysqli_fetch_array($syn);
		...
		<table data-toggle="table"   data-sort-name="name" data-sort-order="desc">
			<thead>
			<tr>
			<th data-sortable="true">Outstanding Process WO</th>
			<th data-sortable="true">Jumlah</th>
			</tr>
			</thead>
			<tr>
				<td class="edit-record13">Hold</td>
				<td><?php echo $datsyn['nowo']; ?></td>
			</tr>
	...
	<script src="js1/bootstrap-table.js"></script>
        ...
        $(function(){
            ...
            $(document).on('click','.edit-record12',function(e){
				window.open('s_a_table_os.php?id=pp&stat=hold',
							'winpopup',
							'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=1200,height=500'
							);
            });
            ...
            $(document).on('click','.edit-record13',function(e){
				window.open('s_a_table_os.php?id=wo&stat=hold',
							'winpopup',
							'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=1200,height=500'
							);
            });
...
<?php
	...
	$sqlr	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='finish' and pls='$pls'");
	$data6	=	mysqli_fetch_array($sqlr);
	...
	$sqls	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='on process' and pls='$pls'");
	$data7	=	mysqli_fetch_array($sqls);
	...
	$sqlt	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='hold' and pls='$pls'");
	$data8	=	mysqli_fetch_array($sqlt);
	...
	$sqlu	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='approved by SH' and pls='$pls'");
	$data9	=	mysqli_fetch_array($sqlu);
	...
	$sqlv	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='waiting' and pls='$pls'");
	$data10	=	mysqli_fetch_array($sqlv);
	...
	$sqlw	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo like 'reject%' and pls='$pls'");
	$data11	=	mysqli_fetch_array($sqlw);
	...
	?>
		...
		<tr>
				<td class="edit-record9">Approved by user SH</td>
				<td><?php echo $data9['jml'];?></td>
		</tr>
		<tr>
				<td class="edit-record10">Waiting approve user SH</td>
				<td><?php echo $data10['jml'];?></td>
		</tr>
		<tr>
				<td class="edit-record11">Reject</td>
