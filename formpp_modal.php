...
$sqla3 = mysqli_query($koneksi,"select * from user where username ='".$usernm."'");
$data3 = mysqli_fetch_array($sqla3);	
$dev 	=$data3['dev'];
...
<?php if($sec=="IT"){?>
	<tr>
	<td style="width: 2%;border:0;border-top:0;"></td>
	<th>Downtime</th>: <td><?php echo $data['downtime'];?>  (Mnt)</td>
	</tr>

	<?PHP } ?>
	<?php if(($data['status_pp']=="finish" OR $data['status_pp']=="complete") AND ($usernm=="ssh-it" OR $usernm=="it01")  ){?>
		<!-- <tr>
	...
	<?php
		...
		if($jml['jl'] > 0 ){
 		...
 		}else if($jml['jl'] < 1 and $data['status_pp']=='on process'){
			 ?>
	...
	<?php if($data['status_pp']=="finish" AND $usernm=="ssh-it" ){?>
...
<div class="panel-body">
	<div >
		<?php
			if (($data['status_hold']=='')&&($data['status_pp']=='waiting')){
		?>
		...
		<!-- <input type="hidden" name="txtkerusahan" value="<?php echo $data['kerusakan']; ?>">
