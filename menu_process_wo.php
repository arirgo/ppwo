			...
			<ul class="breadcrumb">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<div class="modal fade" id="woModal" tabindex="-1" role="dialog" aria-labelledby="twoModalLabel" aria-hidden="true">
					<div class="modal-dialog" style="width:580px">
					<div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                        <h4 class="modal-title" id="twoModalLabel">Input Status Hold</h4>
                    </div>
...
<thead>
 <tr>
							...
							require_once("config.php");
								...
								$nama = $isi['nama'];
								$result="SELECT * FROM tbl_wo where diterima in ('".$nama."','Maman Nurohman') and status_wo IN ('approved by IT','on process','hold') order by tgl_permohonan ASC";
								   ...
								   // mengulangi data agar tidak hanya 1 yang tampil
									...
									<?php
										...
										if($data['status_wo']=='waiting'){
												echo '<td><form name="f_take" action="take_wo.php" method="POST"><input type="hidden" name="txttake" value="'.$data['no_wo'].'"><input type="hidden" name="txtadmin" value="'.$nama.'"><button type="submit" class="btn btn-danger btn-xs" disabled>TAKE</button></form></td>';
											}else if($data['status_wo']=='approved by SH'){
											...
											}else if($data['status_wo']=='approved by IT'){
											echo'<td><a href="#" class="tambah-plan" data-id="'.$data['no_wo'].'" ><button type="button" class="btn btn-inverse btn-xs">PLANING</button></a></td>';
											...
											// echo '<td><form name="f_take" action="process_wo.php" method="POST"><input type="hidden" name="txttake" value="'.$data['no_wo'].'"><input type="hidden" name="txtadmin" value="'.$nama.'"><button type="submit" class="btn btn-danger btn-xs">PROCESS</button></form></td>';
												...
												?>
												<td>
													<a href="#" class="edit-finish" data-id="<?php echo $data['no_wo']; ?>" ><button type="submit" class="btn btn-danger btn-xs">FINISH</button></a>
													<a href="#" class="edit-hold" data-id="<?php echo $data['no_wo']; ?>" ><button type="submit" class="btn btn-inverse btn-xs">HOLD</button></a>
													<a href="#" class="tambah-plan" data-id="<?php echo $data['no_wo'];?>" ><button type="button" class="btn btn-inverse btn-xs"><i class="ace-icon fa  glyphicon-plus bigger-110"></i>MODUL</button></a>
												</td>
												<?php
											}else if($data['status_wo']=='hold'){
												echo '<td><form name="f_release" action="release_wo.php" method="POST"><input type="hidden" name="txtrelease" value="'.$data['no_wo'].'"><input type="hidden" name="txtadmin" value="'.$nama.'"><button type="submit" class="btn btn-purple btn-xs">RELEASE</button></form></td>';
