											...
											<div class="widget-body">
												<div class="widget-main">
													...
													<?php
														...
														$ceknilai_analisis 	=mysqli_query($koneksi,"SELECT sum(nilai_wo.analisis)as ttlanal,count(*) as ttldata FROM tbl_wo INNER JOIN nilai_wo ON tbl_wo.no_wo = nilai_wo.no_wo where tbl_wo.diterima='$user' and tgl_m_kerja between '$tgl1' and '$tgl2'");
														$dataanalisis		=mysqli_fetch_array($ceknilai_analisis);
														$ttlanalisis		=$dataanalisis['ttlanal']/$dataanalisis['ttldata'];

														$ceknilai_programing =mysqli_query($koneksi,"SELECT sum(nilai_wo.programing)as ttlprog,count(*) as ttldata FROM tbl_wo INNER JOIN nilai_wo ON tbl_wo.no_wo = nilai_wo.no_wo where tbl_wo.diterima='$user' and tgl_m_kerja between '$tgl1' and '$tgl2'");
														$dataprograming		 =mysqli_fetch_array($ceknilai_programing);
													    $ttlprograming		 =$dataprograming['ttlprog']/$dataprograming['ttldata'];

														$ceknilai_dokumentasi =mysqli_query($koneksi,"SELECT sum(nilai_wo.dokumentasi)as ttldok,count(*) as ttldata FROM tbl_wo INNER JOIN nilai_wo ON tbl_wo.no_wo = nilai_wo.no_wo where tbl_wo.diterima='$user' and tgl_m_kerja between '$tgl1' and '$tgl2'");
														$datadokumentasi	  =mysqli_fetch_array($ceknilai_dokumentasi);
														$ttldokumentasi		  =$datadokumentasi['ttldok']/$datadokumentasi['ttldata'];

														$ceknilai_point 	  =mysqli_query($koneksi,"SELECT sum(nilai_wo.point)as ttlpoint,count(*) as ttldata FROM tbl_wo INNER JOIN nilai_wo ON tbl_wo.no_wo = nilai_wo.no_wo where tbl_wo.diterima='$user' and tgl_m_kerja between '$tgl1' and '$tgl2'");
														$datapoint			  =mysqli_fetch_array($ceknilai_point);
														$hasilpoint			  =(($datapoint['ttlpoint']/$datapoint['ttldata'])* 33.3);
														if($hasilpoint >99)
														{	$ttlpoint="100";}else{$ttlpoint=$hasilpoint;}
													
													
														$ttldown  =round(strval($pp['down']/60),1);
														...
														$ttlpp			=$pp['ttl_pp'];
														$ttlwo			=$wo['ttl_wo'];
														$ttlnilai_pp	=round(($nilai_pp['nilaipp_ku']/$nilai_pp['jmlpp_ku']),1);
													...
													<div class="clearfix">
														...
														<div class="grid3">
															<span class="yellow"><b><a class="edit-tp" data-id="<?php echo "".$ed.""; ?>" style=" color:#32CD32">Nilai PP</a></b></span>
														...
														<div class="grid3">
															<span class="pink"><b><a class="edit-tp" data-id="<?php echo "".$ed.""; ?>" style=" color:#FFD700;">Analisis</a></b></span>
															<span class="bigger pull-right"><?php echo $ttlanalisis; ?></span>
															<input type="hidden" name="ttlanalisis" id="ttlanalisis<?php echo $no;?>" value="<?php echo $ttlanalisis;?>">
														</div>
														<div class="grid3">
															<span class="pink"><b><a class="edit-tp" data-id="<?php echo "".$ed.""; ?>" style=" color:#4B0082;">Programing</a></b></span>
															<span class="bigger pull-right"><?php echo $ttlprograming; ?></span>
															<input type="hidden" name="ttlprograming" id="ttlprograming<?php echo $no;?>" value="<?php echo $ttlprograming;?>">
														</div>
														<div class="grid3">
															<span class="pink"><b><a class="edit-tp" data-id="<?php echo "".$ed.""; ?>" style=" color:#D2691E;">Dokumentasi</a></b></span>
															<span class="bigger pull-right"><?php echo $ttldokumentasi; ?></span>
															<input type="hidden" name="ttldokumentasi" id="ttldokumentasi<?php echo $no;?>" value="<?php echo $ttldokumentasi;?>">
														</div>
														<div class="grid3">
															<span class="pink"><b><a class="edit-tp" data-id="<?php echo "".$ed.""; ?>" style=" color:#87CEEB;">Point</a></b></span>
															<span class="bigger pull-right"><?php echo $ttlpoint; ?></span>
															<input type="hidden" name="ttlpoint" id="ttlpoint<?php echo $no;?>" value="<?php echo $ttlpoint;?>">
														</div>
	...
	<script src="js1/bootstrap-table.js"></script>
		...
		for(var i=1;i<=jmluser;i++)
			{	
				...
				var ttlanalisis=$('#ttlanalisis'+i).val();
				var ttlprograming=$('#ttlprograming'+i).val();
				var ttldokumentasi=$('#ttldokumentasi'+i).val();
				var ttlpoint=$('#ttlpoint'+i).val();
			  ...
			  var data = [
				...
				{ label: "NILAI PP",  data:ttlnilaipp, color: "#32CD32"},
				{ label: "ANALISIS",  data:ttlanalisis, color: "#FFD700"},
				{ label: "PROGRAMING",  data:ttlprograming, color: "#4B0082"},
				{ label: "DOKUMENTASI",  data:ttldokumentasi, color: "#D2691E"},
				{ label: "POINT",  data:ttlpoint, color: "#87CEEB"},
			  ]
