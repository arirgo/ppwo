...
<?php
	...
	$sqlm	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='finish'");
	$data1	=	mysqli_fetch_array($sqlm);
	$sqln	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='on process'");
	$data2	=	mysqli_fetch_array($sqln);
	$sqlo	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='hold'");
	$data3	=	mysqli_fetch_array($sqlo);
	$sqlp	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='approved by SH'");
	$data4	=	mysqli_fetch_array($sqlp);
	$sqlq	=	mysqli_query($koneksi,"select status_wo, count(status_wo) as jml from tbl_wo where status_wo='waiting'");
...
<?php include "menu-bawah.php"; ?>
	...
	<!--
				<td><a href = "javascript:  window.open('s_a_table_wo.php?n=<?php //echo $dataq['status_pp']; ?>', 'winpopup', 'toolbar=no,statusbar=no,menubar=no,resizable=yes,scrollbars=yes,width=1200,height=500');" class = "btn btn-primary">Complete</a></td>
-->
